...
volatile bool     perfil_alterado    = false;
...
void loop() {
    // Processa log de alteração de perfil
    if (perfil_alterado) {
        noInterrupts();
        uint8_t perfil = perfil_atual;
        perfil_alterado = false;
        interrupts();

        Serial.print(F("[CAN] Comando recebido: Perfil alterado para "));
        if (perfil == 1) Serial.println(F("ESPORTIVO (SPT)"));
        else if (perfil == 2) Serial.println(F("NORMAL (NOR)"));
        else if (perfil == 3) Serial.println(F("ECONOMICO (ECO)"));
    }
